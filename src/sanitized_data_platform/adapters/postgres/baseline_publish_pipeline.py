...
class PostgreSQLBaselinePublishPipelineAdapter(PostgreSQLArtifactPublishPipelineAdapter):
    def __init__(
        self,
        *,
    ...
    def execute(
        ...
        *,
        ...
        if baseline is None:
            raise DomainError("PostgreSQL baseline publish requires a selected sanitized baseline.")
        if target.engine_type != DatabaseEngine.POSTGRES:
            raise DomainError(
                "PostgreSQL baseline publish pipeline adapter can only execute postgres targets."
            )
        ...
        try:
            for asset in assets:
                if asset.artifact_format != ExtractionArtifactFormat.JSONL:
                    raise DomainError(
                        "PostgreSQL baseline publish currently supports only JSONL baseline assets."
                    )
                if not asset.root_object_id.startswith("table:"):
                    raise DomainError(
                        "PostgreSQL baseline publish currently supports only root-table baseline assets."
                    )
                schema_name, table_name = self._parse_root_table(asset.root_object_id)
        ...
        return {
            "baselineStrategy": "postgres-materialized-baseline",
