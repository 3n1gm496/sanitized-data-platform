...
from sanitized_data_platform.domain.entities import ExtractionJob, ExtractionPlan, TransformationPolicy
...
class OracleExtractionPipelineAdapter:
    ...
    def execute(self, *, job: ExtractionJob, plan: ExtractionPlan) -> dict[str, object]:
        ...
        if source.engine_type != DatabaseEngine.ORACLE:
            raise DomainError(
                "Oracle extraction pipeline adapter can only execute oracle data sources."
            )

        owner, table_name = self._parse_table_root(source.source_id, plan.root.object_id)
        available_columns = self._list_table_columns(source.endpoint, owner, table_name)
        ...
        where_sql, params = self._build_where_clause(plan, allowed_columns=available_columns)
        count_query = f'SELECT COUNT(*) FROM "{owner}"."{table_name}"{where_sql}'
        ...
        summary: dict[str, object] = {
            "extractionStrategy": "oracle-table-root",
            "rootObjectId": plan.root.object_id,
            "rootTable": f"{owner}.{table_name}",
    ...
    def _collect_artifact_metadata(self, *, artifact_path: str, column_count: int) -> dict[str, object]:
    ...
    def _build_where_clause(
        ...
        *,
        ...
        for index, criterion in enumerate(plan.root.criteria, start=1):
            ...
            if sql_operator is None:
                raise DomainError(
                    f"Unsupported extraction operator for oracle execution: {criterion.operator}"
                )
            if criterion.operator.lower() == "ilike":
                clauses.append(f'UPPER("{field_name}") {sql_operator} UPPER(:{index})')
            else:
                clauses.append(f'"{field_name}" {sql_operator} :{index}')
            params.append(criterion.value)
        return f" WHERE {' AND '.join(clauses)}", tuple(params)

    def _execute_count(self, endpoint: str, query: str, params: tuple[Any, ...]) -> int:
    ...
    def _execute_rows(
        ...
        return [{column_names[index]: value for index, value in enumerate(row)} for row in rows]
            ...
            with os.fdopen(fd, "w", encoding="utf-8") as handle:
                while True:
                    ...
                    unsupported_types.update(
                        str(value) for value in batch_summary["unsupportedTransformationTypes"]
                    )
    ...
    def _build_select_query(
        ...
        *,
        ...
        if row_limit is not None:
            next_index = len(where_params) + 1
            limit_sql = f" FETCH FIRST :{next_index} ROWS ONLY"
            params = (*where_params, row_limit)
        query = (
            f'SELECT {projected_columns} FROM "{owner}"."{table_name}"'
            f"{where_sql}{order_sql}{limit_sql}"
        )
        return query, params

    def _list_table_columns(self, endpoint: str, owner: str, table_name: str) -> list[str]:
        rows = self._fetch_rows(endpoint, self._LIST_TABLE_COLUMNS_SQL, (owner, table_name))
        columns = [str(row[0]) for row in rows]
        if not columns:
            raise DomainError(
                f"No columns available for oracle extraction root table: {owner}.{table_name}"
            )
        for name in columns:
    ...
    def _list_primary_key_columns(self, endpoint: str, owner: str, table_name: str) -> list[str]:
    ...
    def _resolve_selected_columns(self, *, plan: ExtractionPlan, available_columns: list[str]) -> list[str]:
    ...
    def _fetch_rows(self, endpoint: str, query: str, params: tuple[Any, ...]) -> list[tuple[Any, ...]]:
        connection = self._connect(endpoint)
        cursor = connection.cursor()
        try:
            cursor.execute(query, params)
    ...
    def _parse_table_root(self, source_id: str, root_object_id: str) -> tuple[str, str]:
        ...
        if not root_object_id.startswith(prefix):
            raise DomainError(
                "Oracle extraction pipeline currently supports only table-root object ids from canonical metadata discovery."
            )
        ...
        if not self._SAFE_IDENTIFIER.fullmatch(owner):
            raise DomainError(f"Unsupported schema name in root object id: {owner}")
