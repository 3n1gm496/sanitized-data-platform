...
class OracleBaselineRefreshPipelineAdapter:
    def __init__(
        self,
        *,
        ...
        self._clock = clock
        self._extraction = OracleExtractionPipelineAdapter(
    ...
    def execute(
        ...
        *,
        ...
        if source.engine_type != DatabaseEngine.ORACLE:
            raise DomainError(
                "Oracle baseline refresh pipeline adapter can only execute oracle data sources."
            )
        if profile.dataset_mode != DatasetMode.FULL_CLONE:
            raise DomainError(
                "Oracle baseline refresh currently supports only full-clone dataset profiles."
            )
        ...
        result: dict[str, object] = {
            "refreshStrategy": "oracle-materialized-baseline",
